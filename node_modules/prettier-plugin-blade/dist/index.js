...
var Directives2 = class _Directives {
...
var TreeBuilder = class {
  constructor(tokens, source, directives = Directives2.withDefaults()) {
    this.tokens = tokens;
    this.source = source;
...
function parse(text, options2) {
  ...
  const directives = Directives2.withDefaults(syntaxProfile.treeDirectives);
...
// src/print/index.ts
import { doc as doc12 } from "prettier";
...
var HTML_TAGS = new Set(htmlTags);
...
function getIgnoreCommentKind(node) {
  ...
  if (node.kind === 12 /* Comment */) {
    ...
    value = match?.[1] ?? null;
  } else if (node.kind === 15 /* BladeComment */) {
    const text = fullText(node);
    const match = text.match(/^\{\{--\s*([\s\S]*?)\s*--\}\}$/s);
    value = match?.[1] ?? null;
  }
...
import { doc as doc5 } from "prettier";
...
function formatDirectivePart(part, node, options2) {
  ...
  const spacingMode = isComponentAttributeName(node, options2) ? "none" : getDirectiveArgSpacingMode(options2);
...
function replaceEndOfLine(text, replacement = doc.builders.literalline) {
  const parts = [];
...
// src/print/tag.ts
var { indent, line, softline, hardline, join } = doc2.builders;
...
// src/print/children.ts
var { breakParent, group, hardline: hardline2, softline: softline2, line: line2, dedentToRoot } = doc4.builders;
function getEndLocation(node) {
  if (node.kind === 1 /* Element */ && !node.hasClosingTag && node.children.length > 0) {
...
function shouldKeepInlineIgnoreBoundary(prev, next) {
...
function printChild(childPath, options2, print) {
  ...
  if (hasPrettierIgnore(child) && ignoreMode) {
    ...
    return [
      ...
      ignoreMode === "range" ? dedentToRoot(replaceEndOfLine(preservedText)) : replaceEndOfLine(preservedText),
...
function printChildren(path, print, options2) {
  ...
  if (forceBreakChildren(node)) {
    return [
      ...
      ...path.map((childPath) => {
        ...
        return [
          !prevBetweenLine ? "" : [prevBetweenLine, forceNextEmptyLine(childNode.prev) ? hardline2 : ""],
          printChild(childPath, options2, print)
        ];
      }, "children")
    ];
  }
  const needsGroupIds = node.children.some((child) => !isTextLikeNode(child));
  const groupIds = needsGroupIds ? node.children.map(() => /* @__PURE__ */ Symbol("")) : [];
  return path.map((childPath, childIndex) => {
      ...
      if (childNode.prev && isTextLikeNode(childNode.prev)) {
        ...
        if (prevBetweenLine2) {
          if (forceNextEmptyLine(childNode.prev)) {
            return [hardline2, hardline2, printChild(childPath, options2, print)];
          }
          return [prevBetweenLine2, printChild(childPath, options2, print)];
        }
      }
      return printChild(childPath, options2, print);
    }
    const prevParts = [];
    const leadingParts = [];
    const trailingParts = [];
    ...
    return [
      ...
      group([
        ...leadingParts,
        group([printChild(childPath, options2, print), ...trailingParts], {
...
function printElement(path, options2, print) {
  ...
  const printLineBeforeChildren = () => {
    ...
    if (firstChild.kind === 2 /* Text */ && node.isWhitespaceSensitive && node.isIndentationSensitive) {
      return dedentToRoot2(softline3);
    }
    return softline3;
  };
...
function flattenInlineIntentChildren(childrenDoc) {
  return doc5.utils.mapDoc(childrenDoc, (current) => {
...
// src/print/text.ts
import { doc as doc6 } from "prettier";
var { line: line4, hardline: hardline4, fill, join: join2 } = doc6.builders;
...
import { doc as doc7 } from "prettier";
...
// src/print/directive.ts
var { indent: indent3, hardline: hardline5, dedentToRoot: dedentToRoot3 } = doc7.builders;
...
function getEndLocation2(node) {
  if (node.kind === 1 /* Element */ && !node.hasClosingTag && node.children.length > 0) {
    return Math.max(node.end, getEndLocation2(node.children[node.children.length - 1]));
  }
  return node.end;
}
function printIgnoredDirectiveBodyChild(childPath, options2) {
  ...
  const endLocation = getEndLocation2(child);
  let preservedText = htmlTrimEnd2(
    child.source.slice(
      child.start + (child.prev && needsToBorrowNextOpeningTagStartMarker(child.prev) ? printOpeningTagStartMarker(child).length : 0),
      endLocation - (child.next && needsToBorrowPrevClosingTagEndMarker(child.next) ? printClosingTagEndMarker(child, options2).length : 0)
  ...
  return [
    printOpeningTagPrefix(child, options2),
    ignoreMode === "range" ? dedentToRoot3(replaceEndOfLine(preservedText)) : replaceEndOfLine(preservedText),
    printClosingTagSuffix(child, options2)
  ];
}
...
function renderDirectiveTokens(node, options2) {
  ...
  const tc = node.flat.tokenCount;
  let result = "";
  const argSpacingMode = getEffectiveDirectiveArgSpacingMode(node, options2);
  const start = node.flat.tokenStart;
  ...
  for (let i = 0; i < tc; i++) {
    ...
    if (t.type === 25 /* Directive */) {
      ...
      if (argSpacingMode === "space" && next?.type === 26 /* DirectiveArgs */) {
        result += " ";
      }
      continue;
    }
...
function getEffectiveDirectiveArgSpacingMode(node, options2) {
  if (isDirectiveInBladeComponentAttributeContext(node, options2)) {
    return "none";
  }
...
function printDirectiveBody(branchPath, print, options2) {
  ...
  branchPath.each((childPath, i) => {
    ...
    if (docs.length > 0) {
      ...
      if (prev) {
        docs.push(printBetweenLine2(prev, child));
      }
    }
    if (isBranchNode(child) && child.children.length > 0) {
      docs.push(
        child.kind === 7 /* Directive */ ? renderDirectiveTokens(child, options2) : print(childPath)
      );
      const nestedDocs = printDirectiveBody(childPath, print, options2);
      if (nestedDocs.length > 0) {
        docs.push(indent3([hardline5, nestedDocs]));
      }
    } else {
      docs.push(printDirectiveBodyChild(childPath, print, options2));
    }
  }, "children");
  return docs;
}
function printDirectiveBodyInline(branchPath, print, options2) {
  ...
  branchPath.each((childPath) => {
    const child = childPath.node;
    if (prev !== null) {
      const between = getSourceBetween2(prev, child);
      if (/\s/.test(between)) {
        docs.push(" ");
      }
    }
...
function isDirectiveInElementOpenTag(directive) {
...
function getAttributeContextElement(node) {
  let current = node;
...
function printBetweenLine2(prev, next) {
  ...
  if (getIgnoreCommentKind(prev) === "ignore-start" && nextIgnoreMode === "range" && !hasLineBreakBetweenNodes) {
    return sourceBetween;
  }
  if (prevIgnoreMode === "range" && getIgnoreCommentKind(next) === "ignore-end" && !hasLineBreakBetweenNodes) {
    return sourceBetween;
  }
  ...
  if (isTextLikeNode(prev) && isTextLikeNode(next)) {
    if (prev.isTrailingSpaceSensitive) {
      if (prev.hasTrailingSpaces) {
        ...
        return preferHardlineAsLeadingSpaces(next) ? hardline5 : doc7.builders.line;
      }
      return "";
    }
    if (isEchoLike(prev) || isEchoLike(next)) {
    ...
    return preferHardlineAsLeadingSpaces(next) ? hardline5 : doc7.builders.softline;
  }
  if (!next.isLeadingSpaceSensitive || preferHardlineAsLeadingSpaces(next)) {
    return hardline5;
  }
  if (next.hasLeadingSpaces) {
    return doc7.builders.line;
  }
  return doc7.builders.softline;
}
...
function isBodyLayoutDirective(node) {
  ...
  const directiveToken = findFirstToken(node, 25 /* Directive */);
  if (!directiveToken) {
    return false;
  }
...
// src/print/embed.ts
import { doc as doc11 } from "prettier";
import { format as prettierFormat4 } from "prettier";

// src/print/embed/attribute.ts
import { doc as doc9 } from "prettier";
import { format as prettierFormat2 } from "prettier";
import parseSrcset from "@prettier/parse-srcset";

// src/print/embed/utilities.ts
import { doc as doc8 } from "prettier";
var { group: group3, indent: indent4, softline: softline4 } = doc8.builders;
...
function hasMustacheInterpolation(node) {
...
function isDirectiveWithArgsNode(node) {
  ...
  return findFirstToken2(node, 26 /* DirectiveArgs */) !== null;
}
...
function findFirstToken2(node, type) {
...
function shouldPreferInlineDirectiveArgs(directiveName, payload, options2) {
...
function getPhpFormatCacheKey(snippet, mode, options2, trailingCommaPHPOverride) {
  const opts = options2;
  const optionsKey = safeSerialize({
    mode,
...
function createPhpFormatOptions(options2, plugins, trailingCommaPHPOverride) {
  const baseOptions = {
    ...options2
  };
...
async function formatPhpSnippet(snippet, options2, mode, trailingCommaPHPOverride) {
  const plugins = await resolvePhpPlugins(options2);
  if (!plugins) return null;
  const cacheKey = getPhpFormatCacheKey(snippet, mode, options2, trailingCommaPHPOverride);
  ...
  try {
    const formatted = await prettierFormat(
      snippet,
      createPhpFormatOptions(options2, plugins, trailingCommaPHPOverride)
    );
...
function getDirectiveName(node) {
  const directiveToken = findFirstToken2(node, 25 /* Directive */);
...
function rebuildDirectiveWithFormattedArgs(node, formattedArgs, options2) {
  ...
  const end = start + node.flat.tokenCount;
  const tokens = node.buildResult.tokens;
  const argSpacingMode = getEffectiveDirectiveArgSpacingMode2(node, options2);
  let result = "";
  let sawDirectiveToken = false;
  let sawArgsToken = false;
  for (let i = start; i < end; i++) {
...
function getEffectiveDirectiveArgSpacingMode2(node, options2) {
  if (!isDirectiveComponentAttribute(node, options2)) {
    return getDirectiveArgSpacingMode(options2);
  }
  return "none";
}
function isDirectiveComponentAttribute(node, options2) {
  if (node.kind !== 7 /* Directive */) {
    return false;
  }
  const parent = getAttributeContextElement2(node);
  if (!parent) {
    return false;
  }
  if (!isBladeComponentTagName(parent.fullName, options2)) {
    return false;
  }
  return true;
}
function getAttributeContextElement2(node) {
  let current = node;
  while (current.parent?.kind === 8 /* DirectiveBlock */) {
    current = current.parent;
  }
...
async function formatDirectiveNodeArgs(node, options2) {
  ...
  const argsToken = findFirstToken2(node, 26 /* DirectiveArgs */);
  ...
  for (const attempt of formatAttempts) {
    ...
    const finalPayload = collapsedInlinePayload && shouldPreferInlineDirectiveArgs(directiveName, collapsedInlinePayload, options2) ? collapsedInlinePayload : payload;
...
async function formatPhpBlockNode(node, options2) {
  ...
  const startToken = findFirstToken2(node, 29 /* PhpBlockStart */);
  const endToken = findFirstToken2(node, 30 /* PhpBlockEnd */);
...
// src/print/embed/attribute.ts
var { group: group4, join: join3, line: line5, ifBreak: ifBreak3 } = doc9.builders;
function hasMustacheInterpolation2(node) {
...
async function formatAsPhpBoundAttributeValue(value, options2) {
  ...
  try {
    const formatted = await prettierFormat2(
...
var isStyle = (path, options2) => {
  ...
  const name = getStaticAttributeNameLower(node);
  if (!isStaticAttributeValue(node)) return false;
  return name === "style" && !options2.parentParser && !hasMustacheInterpolation2(node);
};
...
var isEventHandler = (path, options2) => {
  ...
  if (!name) return false;
  if (!isStaticAttributeValue(node)) return false;
  return isHtmlEventAttribute(name) && !options2.parentParser && !hasMustacheInterpolation2(node);
};
...
var isColonBoundAttribute = (path, options2) => {
  ...
  if (!isStaticAttributeValue(node)) return false;
  if (hasMustacheInterpolation2(node)) return false;
  const name = getAttributeNameRaw(node);
  if (!name) return false;
...
var isClassNames = (path, options2) => {
  ...
  const name = getStaticAttributeNameLower(node);
  if (!isStaticAttributeValue(node)) return false;
  return (name === "class" || name === "classname") && !options2.parentParser && !hasMustacheInterpolation2(node);
};
...
var isPermissionsPolicy = (path, options2) => {
  ...
  const name = getStaticAttributeNameLower(node);
  if (!isStaticAttributeValue(node)) return false;
  return name === "allow" && !options2.parentParser && node.parent !== null && node.parent.fullName.toLowerCase() === "iframe" && !hasMustacheInterpolation2(node);
};
...
var printers = rawPrinters.map(({ test, print: p }) => ({
  test,
...
import { doc as doc10 } from "prettier";
import { format as prettierFormat3 } from "prettier";
var { group: group5, hardline: hardline6, indent: indent5 } = doc10.builders;
...
${lines.map((line6) => `  ${line6}`).join("\n")}
  ...
  let formattedMasked;
  try {
    formattedMasked = await prettierFormat3(
      maskedValue,
...
// src/print/embed.ts
var { breakParent: breakParent3, group: group6, hardline: hardline7, indent: indent6 } = doc11.builders;
...
function handleElementEmbed(path, options2) {
  ...
  if (shouldUseInlineIntentElementEmbed(node, options2)) {
    return async (_textToDoc, _print) => {
      try {
        const delegated = await prettierFormat4(
          fullText(node),
...
// src/print/index.ts
var { group: group7, hardline: hardline8 } = doc12.builders;
...
function genericPrint(path, options2, print) {
  ...
  switch (node.kind) {
    ...
    case 0 /* Root */: {
      ...
      if (children.length === 0) return "";
      return [group7(children), hardline8];
    }
    case 1 /* Element */:
    case 14 /* ConditionalComment */:
